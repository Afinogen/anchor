...
String? parseDateHeaderText(String text) {
  ...
  final match =
      _bareDate.firstMatch(trimmed) ?? _dashedDate.firstMatch(trimmed);
...
String lineText(Document document, ParsedLine line) => line.length <= 1
    ? ''
    : document.getPlainText(line.startOffset, line.length - 1);

/// Key of the date header at [index], or null when that line is not one.
String? dateHeaderKey(Document document, List<ParsedLine> lines, int index) {
...
Delta? buildChecklistDateGroupDelta(
  ...
  final order = [for (final item in items) (item as ExistingLine).index];
  var moved = false;
